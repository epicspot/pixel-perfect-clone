...
export const audit = {
  // Authentication
  login: (userId?: string) => 
    logAudit({ action: 'LOGIN', description: `Connexion utilisateur` }),
  
  logout: () => 
    logAudit({ action: 'LOGOUT', description: 'Déconnexion utilisateur' }),
  ...
  tripStatusChange: (tripId: number, routeName: string, oldStatus: string, newStatus: string, agencyId?: number | null) =>
    logAudit({
