...
export default function Expeditions() {
  ...
  const { data: shipments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(`
          *,
          trip:trips(id, departure_datetime, route:routes(name)),
  ...
  // Fetch agencies and trips for form
  ...
  const createShipment = useMutation({
    mutationFn: async (data: {
      ...
      // Get agency code for reference
      const agency = agencies?.find(a => a.id === data.departure_agency_id);
    ...
    onSuccess: (data) => {
      ...
      logAudit({
        action: 'EXPENSE_CREATE', // Using existing action type as placeholder
        entityType: 'expense',
        entityId: data.id,
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {shipments?.reduce((sum, s) => sum + (s.status !== 'cancelled' ? s.total_amount : 0), 0).toLocaleString('fr-FR')} F
                  </p>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {shipment.status === 'pending' && (
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => updateStatus.mutate({ id: shipment.id, status: 'in_transit' })}
                            ...
                            {shipment.status === 'in_transit' && (
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => updateStatus.mutate({ id: shipment.id, status: 'delivered' })}
                            ...
                            {(shipment.status === 'pending' || shipment.status === 'in_transit') && (
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => updateStatus.mutate({ id: shipment.id, status: 'cancelled' })}
                                title="Annuler"
                              >
...
function NewShipmentDialog({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!senderName || !receiverName || (!tripId && (!departureAgencyId || !arrivalAgencyId))) {
    ...
    onSubmit({
      ...
      departure_agency_id: parseInt(departureAgencyId),
      arrival_agency_id: parseInt(arrivalAgencyId),
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <Select value={tripId} onValueChange={(v) => {
              setTripId(v);
              // Auto-fill agencies from trip
              const trip = trips.find(t => t.id.toString() === v);
              ...
              <SelectContent>
                <SelectItem value="">Aucun voyage</SelectItem>
                {trips.map((trip) => (
