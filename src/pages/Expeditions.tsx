...
export default function Expeditions() {
  ...
  // Calculs sécurisés pour les stats
  const pendingCount =
    shipments && Array.isArray(shipments) ? shipments.filter((s) => s.status === "pending").length : 0;

  const inTransitCount =
    shipments && Array.isArray(shipments) ? shipments.filter((s) => s.status === "in_transit").length : 0;

  const deliveredCount =
    shipments && Array.isArray(shipments) ? shipments.filter((s) => s.status === "delivered").length : 0;
...
function NewShipmentDialog({
  ...
  useEffect(() => {
    const pricing = pricingData?.find((p: any) => p.type === type);
    if (pricing) {
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t text-sm text-muted-foreground">
            <p>
              Créé le{" "}
              {format(new Date(shipment.created_at), "dd/MM/yyyy à HH:mm", {
                locale: fr,
              })}
            </p>
            {shipment.delivered_at && (
              <p>
                Livré le{" "}
                {format(new Date(shipment.delivered_at), "dd/MM/yyyy à HH:mm", {
                  locale: fr,
                })}
              </p>
            )}
